...
# Model per trigger. Een volledige herplanning verdient het sterkste model;
# een geskipte dag herverdelen niet.
MODEL_BY_TRIGGER = {
...
def generate_plan(
    ...
    *,
    ...
    return _persist(sb, goal, plan, trigger, model, run["id"], cost, status)
...
def _persist(
    ...
    log.info(
        "plan v%d opgeslagen als %s: %d sessies, %d ingrepen, %d voorgestelde regels, $%.4f",
        version,
        status,
        len(sessions),
        len(adjustments),
